...
def verify_availability(hybrid: HybridDesign) -> AvailabilityReport:
    ...
    """
    logger.info("Verifying analytical availability for %s", hybrid.site.site.name)
    try:
        ...
        logger.info(
            "Analytical availability=%.4f target=%.4f meets_target=%s",
            report.achieved,
            report.target,
            report.meets_target,
        )
        ...
        logger.exception("Availability analysis failed for %s", hybrid.site.site.name)
        raise
